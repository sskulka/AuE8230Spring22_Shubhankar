...
class TurtleBot:
    ...
    def move2goal(self):
        ...
        vel_msg = Twist()
        iCounter = 0
        arr = [[5,5],[8,5],[8,8],[5,8],[5,5]]

        while (iCounter < 5) :
            """Moves the turtle to the goal."""
            goal_pose = Pose()
            # Get the input from the user.
            goal_pose.x = float(arr[iCounter][0])
            #goal_pose.x = rospy.get_param('~x')
            goal_pose.y = float(arr[iCounter][1])
            #goal_pose.y = rospy.get_param('~y')

            print("test"+str(goal_pose.x)+" "+str(goal_pose.y))
            ...
            vel_msg.angular.z = 0
            self.velocity_publisher.publish(vel_msg)
            print("the loop is:" + str(iCounter))
            iCounter=iCounter+1
